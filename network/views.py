...
from admindashoard.models import *
from .models import *
...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        suggestions = Interest_neuron.objects.all()
...
def create_post(request):
    if request.method == 'POST':
        ...
        try:
            ...
            interests = Interest.objects.filter(user=request.user, content_txt=text)
            ...
            if interests:
                ...
                interest = Interest.objects.create(user=request.user, content_txt=text, count=1)

            post.enrich_user.add(request.user)
            post.save()    
...
def like_post(request, id):
    if request.user.is_authenticated:
        if request.method == 'PUT':
            ...
            print(post)
            try:
                postlike, created = Postlike.objects.get_or_create(user=request.user, post=post)
                if created:
                    postlike.date_likes = timezone.now()
                    postlike.save()

                
                
                interests = Interest.objects.filter(user=request.user, content_txt=post.content_text )
                ...
                if interests:
                    for interest in interests:
                     ...
                     interest = Interest.objects.create(user=request.user, content_txt=post.content_text, count=1)
...
def unlike_post(request, id):
    if request.user.is_authenticated:
        if request.method == 'PUT':
            ...
            print(post)
            try:
                postlike = Postlike.objects.get(post=post, user=request.user)
                postlike.delete() 
...
def save_post(request, id):
    if request.user.is_authenticated:
        if request.method == 'PUT':
            ...
            print(post)
            try:
                postsaves, created = Save.objects.get_or_create(user=request.user, post=post)
                if created:
                    postsaves.date_saved = timezone.now()
                    postsaves.save()
...
def unsave_post(request, id):
    if request.user.is_authenticated:
        if request.method == 'PUT':
            ...
            print(post)
            try:
                postsaves = Save.objects.get(post=post, user=request.user)
                postsaves.delete() 
    ...
    if request.user.is_authenticated:
        if request.method == 'PUT':
            ...
            if request.user == post.creater:
                try:
                    interest = Interest.objects.get(user=request.user, content_txt=post.content_text)
                    if interest.count != 1:
                        interest.count -= 1
                        interest.save()
                    else:
